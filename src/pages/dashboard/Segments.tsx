...
import {
  ...
  ResponsiveContainer, Legend, ZAxis, BarChart, Bar
} from "recharts";
...
export default function Segments() {
  ...
  useEffect(() => {
    fetch("http://127.0.0.1:5000/segments")
      .then(r => r.json())
      .then(data => { setSegData(data); setLoading(false); })
      .catch(err => { console.error("Segments fetch error:", err); setLoading(false); });
  }, []);

  // Map CSV columns to expected shape
  // customer_segments.csv likely has: Customer_ID, Segment, Recency, Frequency, Monetary
  const segs = useMemo(() => segData.map((s: any) => ({
    id:       s.Customer_ID ?? s.customer_id ?? s.ID,
    segment:  s.Segment     ?? s.segment     ?? "Low Value",
    orders:   s.Frequency   ?? s.frequency   ?? 1,
    revenue:  s.Monetary    ?? s.monetary    ?? s.Total_Spent ?? 0,
    avg:      s.Monetary && s.Frequency ? s.Monetary / s.Frequency : 0,
    recency:  s.Recency     ?? s.recency     ?? 0,
  })), [segData]);

  const grouped = ["High Value", "Mid Value", "Low Value"].map(s => ({
    segment: s,
    data: segs.filter((c: any) => c.segment === s).slice(0, 200)
  }));

  const profitability = ["High Value", "Mid Value", "Low Value"].map(s => {
    const arr = segs.filter((c: any) => c.segment === s);
    return {
      segment:   s,
      revenue:   Math.round(arr.reduce((a: number, c: any) => a + c.revenue, 0)),
      customers: arr.length,
      avg:       Math.round(arr.reduce((a: number, c: any) => a + c.avg, 0) / (arr.length || 1)),
    };
  });

  const strategies = [
    { segment: "High Value", color: "hsl(var(--chart-1))", title: "Cultivate VIP loyalty",
      copy: "Deploy concierge-tier service, exclusive previews, and white-glove support. These accounts represent ~60% of revenue — protect retention with targeted rewards." },
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Behavioral clustering by spend and order frequency, with segment strategy."
      />

      <Section title="Segment Scatter (Revenue × Orders)"
        description="Each point is a customer · color = K-Means cluster">
        <div className="h-96">
          <ResponsiveContainer width="100%" height="100%">
            <ScatterChart>
              <CartesianGrid strokeDasharray="3 3" stroke={chartTheme.grid} />
              <XAxis dataKey="orders"  name="Orders"  stroke={chartTheme.axis} fontSize={11} tickLine={false} axisLine={false} />
              <YAxis dataKey="revenue" name="Revenue" stroke={chartTheme.axis} fontSize={11} tickLine={false} axisLine={false}
                tickFormatter={v => `$${(v/1000).toFixed(0)}k`} />
              <ZAxis range={[40, 140]} />
              <Tooltip contentStyle={chartTheme.tooltip}
                formatter={(v: any, n: string) => n === "revenue" ? fmt(v) : v}
                cursor={{ strokeDasharray: "3 3" }} />
              <Legend wrapperStyle={{ fontSize: 11 }} />
              {grouped.map(g => (
                <Scatter key={g.segment} name={g.segment}
                  data={g.data} fill={COLORS[g.segment]} fillOpacity={0.7} />
              ))}
            </ScatterChart>
          </ResponsiveContainer>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={profitability}>
                ...
                <Bar dataKey="revenue" radius={[8,8,0,0]} fill="hsl(var(--chart-1))" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </Section>
        ...
        <Section title="Segment Insights" description="Customer counts and average order value">
          <div className="space-y-3">
            {profitability.map(p => (
              <div key={p.segment}
                className="flex items-center justify-between p-4 rounded-xl bg-muted/40 border border-border/40">
          ...
          <Card key={s.segment} className="bg-gradient-card border-border/60 shadow-soft">
            <CardContent className="p-5">
              <Badge variant="outline" className="mb-3"
                style={{ borderColor: s.color, color: s.color }}>{s.segment}</Badge>
